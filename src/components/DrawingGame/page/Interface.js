...
export function Interface() {
    ...
    const changeColorPoint = (idPoint, colorCode) => {
        if (point.status) {
            let newTemp = point.list.map((item) =>
                item.id === idPoint && !item.origin ? { ...item, color: colorCode } : item
            );
    ...
    const nextQuiz = () => {
        ...
        setTimeout(() => {
            ...
            setResult((prev) => ({
                ...prev,
                wrong: prev.wrong + showAnswer(answer[numberQuiz.current - 1].answer, choose).unvalid.length,
            }));
    ...
    const clearAnswer = () => {
        if (point.status) {
            let newTemp = point.list.map((item) => ({
                ...item,
                color: item.id === answer[numberQuiz.current - 1].origin ? " bg-black" : " bg-slate-200",
            }));
    ...
    return (
        ...
        <div className="w-full h-screen flex flex-col items-center justify-center  from-slate-200 to-black" style={{backgroundColor:'#fff'}}>
            <NavLink to="/">
                    <img className="logo-image" src="assets/img/DatLaptoplogo.png"></img>
                </NavLink>
            <div className="w-[900px] h-10 flex items-center justify-between">
                ...
                <div className="fix-margin flex items-center justify-center text-xl text-white font-bold bg-slate-500 h-full px-10 border-2 border-white border-solid" style={{marginLeft: '-95px'}}>
                    DRAWING
                </div>
            ...
            <div className="w-[900px] flex items-center justify-between my-20 ">
                <Sample id={numberQuiz.current < numberQuiz.total ? numberQuiz.current : numberQuiz.total} />
                <div className="relative w-[402px] h-[408px] bg-white grid grid-cols-10 gap-8 z-[2]">
                    {isSubmit && (
                        <img
            ...
            <div className={`flex items-center${isFinish ? " justify-center" : " justify-between"} w-[900px]`}>
                {!isFinish && (
                    <button
                ...
                {!isFinish && (
                    <div className="text-xl font-bold text-black">
                        ...
                        {isSubmit ? showAnswer(answer[numberQuiz.current - 1].answer, choose).unvalid.length : 0}
                    </div>
                )}
                {!isFinish && (
                    <button
